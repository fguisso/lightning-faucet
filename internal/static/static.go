...
func (s *storage) Add(filetype string, filename string, content []byte) {
	if filetype == "static" {
		s.assets[filename] = content
	} else {
		s.templates[filename] = content
	}
}
...
func Add(filetype string, filename string, content []byte) {
	s.Add(filetype, filename, content)
}
